...
if gotPy2exe:
    ...
    class BuildInstaller(py2exe.build_exe.py2exe):
        ...
        user_options += [
            ("makensis=", None, "path to makensis.exe, the NSIS compiler."),
        ]

        def isSystemDLL(self, pathname):
            # Trap and flag as non-system DLLs those that py2exe
            # would otherwise get incorrect.
            if os.path.basename(pathname).lower() in ["sdl_ttf.dll"] or os.path.basename(pathname).lower() in ["libogg-0.dll"]:
                return 0
            else:
